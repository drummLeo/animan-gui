...
class Anime:
    ...
    def get_episodes(self, init=False):
        self.episodes.clear()
        self.season_breaks = {}  # {ep_index: season_number}
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
        soup = BeautifulSoup(requests.get(self.link, headers=headers).content, "html.parser")

        seen_urls = set()
        collected = []  # list of (season_num, title, url)
        season_num = 0
        ...
        for ul in soup.select('ul.episodios'):
            ...
            for a in ul.select('li a'):
                ...
                aux = span.text.strip() if span else ''
                new_in_season.append((season_num + 1, aux, url))
            if new_in_season:
                season_num += 1
        ...
        total = len(collected)
        prev_season = None
        for i, (snum, aux, url) in enumerate(collected):
            ep_title = aux if aux else f"Episódio {i + 1}"
            ep_name = f'{i + 1}. "{ep_title}"'
            if total >= 10 and i + 1 <= 9:
                ep_name = ' ' + ep_name
            if snum != prev_season:
        ...
        if not init:
            with open(self.file_name, 'w') as file:
                anime_info = {'name': self.name, 'link': self.link,
                              'last_episode': self.last_episode, 'ep_num': total,
